""" Development code for CANDLE Milestone 14. """
...
def parse_args(args):
    ...
    parser.add_argument('-ml', '--model_name', default='nn_reg0', choices=['lgb_reg', 'nn_reg0'],
                        type=str, help='ML model for training (default: nn_reg0).')
...
def create_outdir(outdir, args, src):
    ...
    l = [('cvf'+str(args['cv_folds']))] + [args['target_name']] 
...
def run(args):
    ...
    for fold, (tr_k, vl_k) in enumerate(zip( tr_dct.keys(), vl_dct.keys() )):
        ...
        ep_best = hh.loc[ hh[eval_metric]==hh[eval_metric].min(), 'epoch' ].values[0]
